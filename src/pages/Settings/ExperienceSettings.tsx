...
export const ExperienceSettings = ({ settings, updateSettings }: Props) => {
  ...
  const sync = (next: SettingsType["experience"]) => {
    updateSettings("experience", next);
  };

  const handleEntryChange = (
    idx: number,
    field: keyof Omit<SettingsType["experience"][number], "date" | "notes">,
    value: string
  ) => {
  ...
  const addEntry = () => {
    const newEntry: SettingsType["experience"][number] = {
      jobTitle: "",
      company: "",
      location: "",
      description: "",
